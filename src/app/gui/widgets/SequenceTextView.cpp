...
private:
    ...
    void addBackgroundSelection()
    {
        ...
        backgroundSelection.start_ = 0;
        backgroundSelection.stop_ = view_->sequence_.length();
        backgroundSelection.backColor_ = Qt::white;
        backgroundSelection.color_ = Qt::black;
    ...
    {
        if (currentIndex_ > currentSelection_->stop_)
        {
            selectionStack_.pop();
            while (selectionStack_.top().stop_ < currentIndex_)
            {
                selectionStack_.pop();
            }
    ...
    void computeIfInNextSelection()
    {
        ...
        if (currentIndex_ == nextSelectionStart_)
        {
            ...
            while (++nextSelectionIndex_ < selections.length())
            {
                ...
                if (currentIndex_ == nextSelection.start_)
                {
                    selectionStack_.push(nextSelection);
                }
                else
                {
                    nextSelectionStart_ = nextSelection.start_;
                    break;
                }
            }
    ...
    void initializeSelectionStack()
    {
        ...
        {
            if (selections[nextSelectionIndex_].start_ > currentIndex_)
            {
                nextSelectionStart_ = selections[nextSelectionIndex_].start_;
                break;
            }

            if (selections[nextSelectionIndex_].stop_ >= currentIndex_)
            {
                selectionStack_.push(selections[nextSelectionIndex_]);
            }
        }
    ...
    {
        painter_->fillRect(x_, y_ , view_->colPadding_, view_->lineHeight_, currentSelection_->backColor_);
    ...
    void paintCurrent()
    {
        painter_->drawPixmap(
                    ...
                    cache_->getCharImage(view_->sequence_[currentIndex_],
                                         currentSelection_->backColor_,
                                         currentSelection_->color_));
        x_ += view_->charWidth_;
    }
    ...
    void paintLeftIndex()
    {
        ...
        painter_->setPen(currentSelection_->color_);
    }
...
{
    color_ = Qt::black;
    backColor_ = QColor("#A8CDF1");
}

Selection::Selection(const ClosedIntRange &range)
    : start_(range.begin_ - 1),
      stop_(range.end_ - 1)
{
    color_ = Qt::black;
    backColor_ = QColor("#A8CDF1");
...
{
    return start_ < selection.stop_ && stop_ > selection.start_;
}
    ...
    QAbstractScrollArea(parent)
{
    ...
    connect(actionCopy_, SIGNAL(triggered()), SLOT(copyToClipboard()));
}
...
void SequenceTextView::addSelection(const ClosedIntRange &range)
{
    Selection sel;
    sel.start_ = range.begin_ - 1;
    sel.stop_ = range.end_ - 1;
    addSelection(sel);
}
...
void SequenceTextView::adjustSelectionToIndex(int index)
{
    ...
    current->start_ = qMax(0, qMin(index, mouseDownIndex_));
    current->stop_ = qMin(sequence_.length() - 1, qMax(index, mouseDownIndex_));
}
...
void SequenceTextView::copyToClipboard()
{
    if (selections_.count())
    {
        ...
        qApp->clipboard()->setText(sequence_.mid(current.start_, (current.stop_ - current.start_) + 1));
    }
}
...
bool SequenceTextView::ensureSelectionExists(int index)
{
    if (index >= 0 && !selections_.count())
    {
        ...
        Selection current;
        current.start_ = mouseDownIndex_;
        current.stop_ = mouseDownIndex_;
...
void SequenceTextView::insertInOrder(const Selection &selection)
{
    ...
    {
        if (selections_[i].start_ > selection.start_)
        {
...
void SequenceTextView::mouseReleaseEvent(QMouseEvent *e)
{
    if (allowSelect() && e->button() == Qt::LeftButton)
...
Selection SequenceTextView::sanitizeSelection(const Selection &selection) const
{
    ...
    sanitizedSelection.start_ = qMax(0, sanitizedSelection.start_);
    sanitizedSelection.stop_ = qMin(sequence_.length() - 1, sanitizedSelection.stop_);
...
void SequenceTextView::selectBlockContainingIndex(int index)
{
    if (index >= 0)
    {
        ...
        selectedBlock.start_ = index - (index % COL_SIZE);
        selectedBlock.stop_ = selectedBlock.start_ + COL_SIZE - 1;
