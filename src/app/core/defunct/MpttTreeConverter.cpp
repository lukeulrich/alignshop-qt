...
/**
  ...
  * @see fromMpttVector()
  */
QVector<MpttNode> MpttTreeConverter::toMpttVector(ITreeNode *treeNode)
{
...
/**
  ...
  */
ITreeNode *MpttTreeConverter::fromMpttVector(QVector<MpttNode> &mpttNodeVector)
{
    ...
    ITreeNode *root = mpttNodeVector.first().treeNode_;
...
/**
  ...
  */
int MpttTreeConverter::recurseBuildMpttVector(ITreeNode *node, int left, QVector<MpttNode> &mpttNodeVector)
{
    ...
    for (int i=0, z=node->childCount(); i<z; ++i)
        right_value = recurseBuildMpttVector(node->childAt(i), right_value, mpttNodeVector);
...
void MpttTreeConverter::validateMpttVector(const QVector<MpttNode> &mpttNodeVector)
{
    ...
    QVector<MpttNode> parentMptts;
    QHash<ITreeNode *, bool> seenTreeNode;
    ...
    for (int i=1, z=mpttNodeVector.size(); i<z; ++i)
    {
        ...
        ITreeNode *treeNode = mpttNode.treeNode_;
